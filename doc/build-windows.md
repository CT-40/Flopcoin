...
Below are some notes on how to build Flopcoin Core for Windows.
...
To build Flopcoin from source code, you'll need the source code. Either check it out via `git` or download
a zip file. (Look at the green "<> Code" button on [the Flopcoin GitHub repository](https://github.com/Flopcoin/Flopcoin/)).
...
Note that for WSL v1 the Flopcoin Core source path MUST be somewhere in the default mount file system, for
example /usr/src/flopcoin, AND not under, for example, /mnt/d/flopcoin.
...
Ensure that this toolchain can build for `posix` (else you may have compilation errors for Flopcoin's dependencies):
...
way. This will install to `c:\workspace\flopcoin`, for example:

    make install DESTDIR=/mnt/c/workspace/flopcoin
