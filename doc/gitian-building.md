...
*Setup instructions for a Gitian build of Flopcoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Flopcoin
...
- [Building Flopcoin Core](#building-flopcoin-core)
...
Clone the git repositories for flopcoin and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/flopcoin/flopcoin.git
git clone https://github.com/flopcoin/gitian.sigs.git
```
...
in the flopcoin repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Flopcoin Core
----------------

To build Flopcoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the flopcoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/flopcoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/flopcoin/flopcoin
...
For example:
```bash
URL=https://github.com/laanwj/flopcoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit flopcoin=${COMMIT} --url flopcoin=${URL} ../flopcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit flopcoin=${COMMIT} --url flopcoin=${URL} ../flopcoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit flopcoin=${COMMIT} --url flopcoin=${URL} ../flopcoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the flopcoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../flopcoin/contrib/gitian-descriptors/*|sort|uniq )
...
cd /some/root/path/
git clone https://github.com/flopcoin/flopcoin-detached-sigs.git

BTCPATH=/some/root/path/flopcoin
SIGPATH=/some/root/path/flopcoin-detached-sigs

./bin/gbuild --url flopcoin=${BTCPATH},signature=${SIGPATH} ../flopcoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/flopcoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/flopcoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/flopcoin-osx-build.assert
```
...
[flopcoin/gitian.sigs](https://github.com/flopcoin/gitian.sigs/) repository, or if that's not possible create a pull
