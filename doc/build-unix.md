...
You must install required dependencies to build a basic Flopcoin daemon, optional dependencies may vary according to your requirements.
...
Flopcoin Core's released binaries are built using the "depends system", which
...
**Optional dependencies** :
```bash
# Qt (required for flopcoin-qt GUI)
...
According to installed dependencies, the following steps will compile `flopcoind`, `flopcoin-cli` and `flopcoin-qt`.
...
#### Enable Qt GUI
Create `flopcoin-qt`, the core wallet GUI.
```bash
./configure --with-gui
```
...
By default, **Flopcoin Core expects BerkeleyDB 5.3**.
...
Otherwise, you can build Flopcoin Core from self-compiled [depends](/depends/README.md).
...
least 1.5 GB of memory available when compiling Flopcoin Core.
