...
/**
 * Fees smaller than this (in florins) are considered zero fee (for transaction creation)
...
{
    // Flopcoin: Add an increased fee for each dust output
    return std::max(minTxFee.GetFee(nTxBytes) + GetFlopcoinDustFee(tx.vout, minTxFee), ::minRelayTxFee.GetFee(nTxBytes));
}
