...
CFeeRate GetFlopcoinFeeRate(int priority)
{
    switch(priority)
    {
    case SUCH_EXPENSIVE:
        return CFeeRate(COIN / 100 * 521); // 5.21 FLOP, but very carefully avoiding floating point maths
    case MANY_GENEROUS:
...
const std::string GetFlopcoinPriorityLabel(int priority)
{
    switch(priority)
    {
    case SUCH_EXPENSIVE:
...
CAmount GetFlopcoinMinRelayFee(const CTransaction& tx, unsigned int nBytes, bool fAllowFree)
{
    ...
    nMinFee += GetFlopcoinDustFee(tx.vout, nDustLimit);
...
CAmount GetFlopcoinDustFee(const std::vector<CTxOut> &vout, const CAmount dustLimit) {
    CAmount nFee = 0;
