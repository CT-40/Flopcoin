...
const std::string strMessageMagic = "Flopcoin Signed Message:\n";
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<uint256>& vHashTxnToUncache)
{
...

    {
        ...
        if (fLimitFree && nModifiedFees < GetFlopcoinMinRelayFee(tx, nSize, !fLimitFree))
        {
...
bool CheckTxInputs(const CChainParams& params, const CTransaction& tx, CValidationState& state, const CCoinsViewCache& inputs, int nSpendHeight)
{
        ...
        for (unsigned int i = 0; i < tx.vin.size(); i++)
        {
            ...
            if (coins->IsCoinBase()) {
                // Flopcoin: Switch maturity at depth 145,000
...
void ThreadScriptCheck() {
    RenameThread("flopcoin-scriptch");
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    // Flopcoin: BIP30 has been active since inception
    ...
    // Flopcoin: BIP16 has been enabled since inception
    ...
    CAmount blockReward = nFees + GetFlopcoinBlockSubsidy(pindex->nHeight, chainparams.GetConsensus(pindex->nHeight), hashPrevBlock);
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const CBlockIndex* pindexPrev, int64_t nAdjustedTime)
{
    ...
    // Flopcoin: Disallow AuxPow blocks before it is activated.
    ...
    // Flopcoin: Version 2 enforcement was never used
    ...
    // Flopcoin: Introduce supermajority rules for v4 blocks
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, const CBlockIndex* pindexPrev)
{
    ...
    // Flopcoin: We probably want to disable this
    int nLockTimeFlags = 0;
