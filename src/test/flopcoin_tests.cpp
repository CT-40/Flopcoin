...
BOOST_AUTO_TEST_CASE(subsidy_first_100k_test)
{
    ...
    for (int nHeight = 0; nHeight <= 100000; nHeight++) {
        ...
        CAmount nSubsidy = GetFlopcoinBlockSubsidy(nHeight, params, ArithToUint256(prevHash));
...
BOOST_AUTO_TEST_CASE(subsidy_post_145k_test)
{
    ...
    CAmount nConstantSubsidy = GetFlopcoinBlockSubsidy(600000, mainParams.GetConsensus(600000), prevHash);
    BOOST_CHECK_EQUAL(nConstantSubsidy, 10000 * COIN);

    nConstantSubsidy = GetFlopcoinBlockSubsidy(700000, mainParams.GetConsensus(700000), prevHash);
    BOOST_CHECK_EQUAL(nConstantSubsidy, 10000 * COIN);
}
