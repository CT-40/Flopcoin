...
// NOTE: Unlike wallet RPC (which use BTC values), mining RPCs follow GBT (BIP 22) in using florin amounts
...
UniValue getblocktemplate(const JSONRPCRequest& request)
{
    ...
    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Flopcoin is downloading blocks...");
...
static UniValue AuxMiningCreateBlock(const CScript& scriptPubKey)
{
     ...
     * a single flopcoind instance, for example when a pool runs multiple sub-
